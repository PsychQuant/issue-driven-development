...
從 `$PWD` 往上走找第一個 IDD config —— **新路徑 `.claude/.idd/local.json` 優先**，legacy `.claude/issue-driven-dev.local.json` 次之（對齊 config-protocol mechanism 4，#195）。Stop 在 `$HOME` 或 `/`。
...
無論 E1/E2/E-TP 選了什麼，都把結果寫入**新路徑** `$PWD/.claude/.idd/local.json`（#195：對齊 config-protocol 「new installs MUST write `.claude/.idd/local.json`」；不再寫 legacy `issue-driven-dev.local.json`。既有 legacy config 仍由 walk-up 的 legacy fallback 解析，向後相容）。`mkdir -p .claude/.idd` 後寫：
