...
description: |
  ...
  支援 batch mode（v2.34.0+）：多個 comment 套同一段 edit（如 `comment:NNN comment:MMM --replace --scope whole-comment --body '...'`），每個 comment 仍 per-confirm。 `--replace` 必須帶 `--scope` 或 `--section` per v2.74.0+ BREAKING。
  Use when: 補既有 comment 說明（如圖片下方解釋）、修 typo、標示「此 comment 已被後續 errata 修正」。
  防止的失敗：手動 `gh api PATCH` 字串 escape 錯誤、誤覆蓋未 backup 的原內容。
argument-hint: "comment:<id>[ comment:<id>...]|#issue --last [--append|--replace --scope whole-comment|--replace --section <heading>|--prepend-note] [--body=\"...\"|--body-file=<path>] [--override-user-content --reason=\"...\"] (multi-comment = batch)"
allowed-tools:
  - Bash(gh:*)
  - Read
  - Write
...
`idd-edit comment:NNN comment:MMM --replace --scope whole-comment --body '...'` 把同一段內容套到多個 comment。Edit 是破壞性動作，batch 把破壞範圍放大 N 倍 — preview + per-comment confirm 仍照舊（不允許 `--yes-to-all`），但每個 confirm 後就推進，不需要 N 次重打命令。`--replace` 必須帶 `--scope` 或 `--section` per v2.74.0+ BREAKING（單 comment 或 batch 都一樣）。
...
# v2.74.2+ #137 verify R2 + Codex audit fix: positional shift pattern handles BOTH `--flag=value` AND `--flag value`
# (space) forms. The pre-R2 for-loop only handled `=` form and dropped space-form value as literal "next".
...
BODY_INPUT=""        # via --body / --body-file
REASON_INPUT=""      # for edit metadata
ARG=""               # positional: comment:<id> | #<issue> | comment:<id> comment:<id>... (batch)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --prepend-note)          MODE="prepend-note"; shift ;;

        --scope=*)               SCOPE_FLAG="${1#--scope=}"; shift ;;
        --scope)                 SCOPE_FLAG="$2"; shift 2 ;;

        --section=*)             SECTION_FLAG="${1#--section=}"; shift ;;
        --section)               SECTION_FLAG="$2"; shift 2 ;;

        --override-user-content) OVERRIDE_USER="true"; shift ;;

        --reason=*)              REASON_INPUT="${1#--reason=}"
                                 [ "$OVERRIDE_USER" = "true" ] && OVERRIDE_REASON="$REASON_INPUT"
                                 shift ;;
        --reason)                REASON_INPUT="$2"
                                 [ "$OVERRIDE_USER" = "true" ] && OVERRIDE_REASON="$REASON_INPUT"
                                 shift 2 ;;

        --body=*)                BODY_INPUT="${1#--body=}"; shift ;;
        --body)                  BODY_INPUT="$2"; shift 2 ;;

        --body-file=*)           BODY_INPUT="$(cat "${1#--body-file=}")"; shift ;;
        --body-file)             BODY_INPUT="$(cat "$2")"; shift 2 ;;

        --last)                  LAST="true"; shift ;;
        --repo)                  GITHUB_REPO="$2"; shift 2 ;;
        --cwd)                   CWD="$2"; shift 2 ;;

        --*)                     echo "✗ Refuse: unknown flag '$1'" >&2; exit 1 ;;
        *)                       POSITIONAL+=("$1"); shift ;;
    esac
done

# First positional = target ID (comment:N | #N); later positionals = batch mode targets
ARG="${POSITIONAL[0]:-}"

# NOTE: Two-arg "--flag value" form requires next positional to exist; if user types
# `--scope` with no following arg, bash binds $2 to next flag or empty. The downstream
# Step 1 refuse gate ("--replace requires --scope whole-comment OR --section <heading>")
# catches the case where SCOPE_FLAG ends up empty. Future enhancement: explicit per-flag
# missing-value check (`[ -z "$2" ] && abort "--scope requires a value"`) — not blocker.
...
$AUDIT_MARKER"
    ...
    # Compute level (count of leading # characters). `wc -c` includes trailing newline → subtract 1.
    SECTION_LEVEL=$(echo "$SECTION_FLAG" | grep -oE '^#+' | tr -d '\n' | wc -c | tr -d ' ')
    # e.g. "### Foo" → SECTION_LEVEL=3
    if ! [[ "$SECTION_LEVEL" =~ ^[1-6]$ ]]; then
        echo "✗ Refuse: --section value '$SECTION_FLAG' must start with 1-6 '#' chars (markdown heading)" >&2
        exit 1
    fi

    # Pre-flight: verify section exists in current body with line-anchored exact match.
    # `grep -qFx` (substring → exact LINE match) prevents `--section "## Foo"` from passing
    # pre-flight when body has `## Foo Bar` (substring match). Splice uses `$0 == section`
    # exact-line so without -x the pre-flight could pass + splice produce zero replacement
    # (silent partial failure per R2 Codex F2 finding).
    if ! grep -qFx "$SECTION_FLAG" "$BACKUP_FILE"; then
        echo "✗ Refuse: section '$SECTION_FLAG' not found as exact-line heading in comment $COMMENT_ID body. Existing headings:" >&2
    ...
    # Splice: keep content before section + replace section content with new + keep content after.
    # End-of-section detection: next heading at SAME or HIGHER level (i.e. fewer-or-equal '#' chars).
    # `^#{1,N} ` regex matches N or fewer leading '#' followed by space — fires on same-or-higher
    # level heading. Without this, `##` section replacement would terminate at any `###` subheading
    # and leave orphan subsections behind (R2 Codex F3).
    NEW_BODY=$(awk -v section="$SECTION_FLAG" \
                   -v new_content="$BODY_INPUT" \
                   -v marker="$AUDIT_MARKER" \
                   -v level="$SECTION_LEVEL" '
        BEGIN {
            in_section=0
            # Build end-of-section regex: ^#{1,level} (space) — matches same-or-higher level heading
            end_re = "^#{1," level "} "
        }
        $0 == section {
            print $0
        ...
        in_section && $0 ~ end_re { in_section=0 }   # only same-or-higher level heading ends section
        !in_section { print }
    ' "$BACKUP_FILE")
fi
```
...
```
# v2.74.0+ — --replace 必須帶 --scope OR --section per BREAKING discipline
