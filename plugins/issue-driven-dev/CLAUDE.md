...
| `idd-all-chain` | Root issue 衍生鏈被切成 N 個獨立 PR,reviewer 失去 holistic view;手動逐一跑 `/idd-all #M` 太繁瑣 | **chain-solve mode**(v2.55.0+):驅動 root issue + auto-emergent spawn(sub-skill sister sweep / verify follow-up / mid-plan tangential / sister concern)through ONE cluster branch + ONE review PR。Thin shell over `/idd-all`,內部 recursive 用 `--in-chain` flag 推導 4th mode tuple `(direct-commit, unattended)`。Hard caps:depth=2、max-issues=5。Eligibility = same-file OR same-skill OR sister-bug。Verify FAIL halt + preserve commits。停在 verified、永不 auto-close。Spawn manifest at `.claude/.idd/state/chain-spawned-issues.json`,4 sub-skill 共同 conformant write |
...
Hard caps(寫死):depth=2、max-issues=5。Eligibility = `same_file_as_root OR same_skill_as_root OR spawn_kind="sister-bug"`(不 eligible 仍 file 為 follow-up issue,但不 chain solve)。Verify FAIL halt + preserve commits + abort report。永不 auto-close、永不 auto-merge。
