...
- **Discussion-metadata helper for idd-issue (#141 + #142)** — new **Step 3.5** surfaces GitHub sidebar metadata (assignee / label / milestone) for discussion-type issues (advisor briefing / review-feedback / awaiting-decision). A deliberately **Lean-v1** slice: detection via `--discussion` flag + a **default-off**, config-opt-in body heuristic (`discussion_metadata_heuristic`) so standard bug/feature issues are never disturbed; assignee resolved via the #86 registry; label `discussion` (config-overridable `discussion_label`); milestone time-bound-gated. **Surface-only contract**: attended → `AskUserQuestion` per sub-step; **unattended → never auto-sets outward-facing metadata** (surfaces a "suggested (not applied)" block instead). `blocked-by` native-relationship suggestion is documented-deferred (Lean-v1). #142 hardens the label path against blind `--add-label` (GitHub can silently auto-create a default-white label): `label_exists()` pre-add existence check (exact match), verify-by-read after create, and a cache-lag hint. New drift-guard `scripts/tests/discussion-metadata/test.sh` (14 assertions). Via PR #248.
...
- **BREAKING — superpowers becomes a hard install-time dependency; TDD / bug-RCA / pre-completion disciplines delegate to it (#209, v2.90.0)** — `plugin.json` now declares `dependencies: [{name: superpowers, marketplace: claude-plugins-official}]` (auto-installed when installing this plugin; unversioned — the official marketplace publishes no `{name}--v{version}` tag line to constrain against) and the root `marketplace.json` allowlists `claude-plugins-official` via `allowCrossMarketplaceDependenciesOn`. Three delegation sites replace IDD's built-in process narratives with the canonical superpowers skills: `idd-implement` Step 3 TDD loop → `superpowers:test-driven-development`, `idd-implement` Step 5b pre-completion check → `superpowers:verification-before-completion`, `idd-diagnose` bug root-cause framework → `superpowers:systematic-debugging`. IDD keeps its issue-anchored wrapper discipline (commit `Refs #N`, scope guard, Diagnosis Report / Implementation Complete formats). `check-plugin-presence.sh` gains an optional third argument for skill-level pre-flight (highest installed version via `sort -V`, mirroring the pai D1 resolution; skill-name traversal-hardened; 15-assertion fixture suite at `scripts/tests/check-plugin-presence/`). On a missing plugin or missing skill the delegation site **aborts** with the one-step install command (`claude plugin install superpowers@claude-plugins-official`) — no vendored fallback, no silent degrade (owner's hard-dependency ruling, #209 Clarity row 1). Keep list untouched: idd-verify ensemble (pai canonical), worktree isolation, planning. Requires Claude Code v2.1.110+ (dependency resolution) / v2.1.143+ (transitive enable). NEW dev-only project rule `.claude/rules/deep-integration-over-hardcode.md` codifies the「深度整合 >> hard-coded」criteria chain with the pai and superpowers exemplars. Spectra change `idd-depend-on-superpowers`.
...
- **User-facing rule injection — SessionStart hook + canonical rule promotion (#214, v2.91.0)** — the commit issue-reference discipline (#213) now reaches plugin USERS, not just this repo's maintainer sessions. NEW `rules/commit-issue-reference.md` is the user-facing canonical (four iron rules; the dev-layer copy shrinks to a ≤10-line distillation pointing back — no parallel full texts, per the deep-integration anti-duplication criterion). NEW `hooks/hooks.json` + `hooks/session-start-commit-rule.sh` inject a ≤5-line static distillation into every session (the only ambient channel that covers manual commits outside skills — plugin-root CLAUDE.md never loads into project context); the line ceiling and token alignment with the canonical file are hard-locked by the NEW drift-guard suite `scripts/tests/session-start-commit-rule/` (22 assertions). `idd-implement` / `idd-all` commit sections gain skill-scope pointers. The layering criterion itself (beneficiary axis: user-behavior rules ship with the plugin, maintainer-decision rules stay dev-only) is documented as a dev-layer rule for the full re-homing sweep tracked in #215. Spectra change `rules-layering-user-injection`.
...
- **Privacy-scrubbing gate — Phase 1 (#202)** — a repo-aware privacy gate now runs before GitHub issue egress. Detection is **LLM semantic self-review** (no maintained regex denylist / name-detector — design D1); enforcement is a single deterministic choke-point wrapper **`scripts/gh-egress.sh`** (design D2) that all egress routes through instead of raw `gh issue create/comment/edit`. The wrapper (a) **refuses dispatch without a `--scrub-attested <enforce|warn|light>` attestation** (Open Question Q1 resolved: per-call required flag whose value is the resolved repo-visibility strictness — not an env var), and (b) runs a tiny mechanical last-resort net catching **only 2 zero-tolerance literal items** — an absolute `/Users/<name>` home path and verbatim `~/.claude.json` content — with no semantic pattern matching. Strictness is three-level: third-party → **ENFORCE** (block-with-diff, never silent auto-redact — D4), own-public → **WARN**, private → **LIGHT**, driven by folding **`isPrivate`** into the existing `gh repo view --json isFork,parent,viewerPermission` call (D3, zero extra round-trip; resolves the push-permission proxy residue from `add-third-party-clone-setup/design.md:145`). NEW shared policy rule **`rules/privacy-scrubbing.md`** (sibling to `tagging-collaborators.md`) is the contract home. **Phase 1 retrofits `idd-issue` only** (highest-risk authoring path — NEW Step 0.6 gate + every create/comment/edit site incl. the multi-finding Stage 4 dispatch routed through the wrapper); the other 11 egress sites are Phase 2 follow-up (spec covers them, implementation deferred). `sanitize_source_label` (#75) is byte-for-byte unchanged — privacy scrubbing is a new semantic layer on top. Fixture test suite `scripts/tests/gh-egress/` (19 assertions) covers attestation enforcement, the mechanical net, non-over-matching (placeholder / legitimate / semantic-only bodies pass), and byte-preserved pass-through. Spectra change `add-privacy-scrubbing-gate`.
...
- **Config write-back / walk-up unified on `.claude/.idd/local.json`** — every runnable config reader and writer across the idd-* skills now resolves new-path-first with legacy fallback (were inconsistently split: writers on the new path, several readers legacy-only — a split-brain caught in verify). Writers migrated: `idd-issue` Step 0.5.E F, `idd-config init`, `idd-verify` no-config write (E-TP already did). Readers migrated: `idd-issue` Step 0.5.B, `idd-all` Step 0.2, `idd-clarify` Step 2 walk-up, `idd-list` cluster_leader, `idd-config` show/validate. This also fixes a latent #192 gap (E-TP's new-path config was invisible to idd-issue's own legacy-only walk-up → re-detected every run). Legacy `issue-driven-dev.local.json` still resolves. (#195)
