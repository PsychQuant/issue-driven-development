...
| v2.99.0 | 2026-07-18 | **`/idd-ask` — issue 知識庫 grounded 問答**（[#72](https://github.com/PsychQuant/issue-driven-development/issues/72)，PR #266）— surfacing family 第 4 員，鏡像 /spectra-ask：decide-to-search gate（bug 貌問題不觸發 diagnose）→ retrieval delegate idd-find backend → top-N 全文（5/上限 10）→ grounded 合成（blockquote 原問題、claim 必附引用、source priority、分歧 surface、`### Referenced Issues`、查無誠實）。#140 增員程序首戰（Q3 弱命中 judgment 入 family canonical）。新 capability spec idd-ask；37 suites 0 fail。詳見 CHANGELOG.md。 |
| v2.98.0 | 2026-07-18 | **codex 通道完全依賴化**（[#264](https://github.com/PsychQuant/issue-driven-development/issues/264)，PR #265）— vendored `bin/codex-call` **刪除**（落後 pai 四個修正，切換即淨升級）；executable 歸 pai（`MIN_PAI` 2.19.0，codexModel/codexEffort 契約）、治理歸 codex-pro（`MIN_CODEX_PRO` 0.7.0，`defaults.json` + profile.yaml 兩層）；IDD 樹內**零 model pin**，換代只動 codex-pro 一處；superpowers 形狀依賴接線（fail-fast、無 soft fallback）。上游先行：codex-pro 0.7.0 + pai 2.19.0。詳見 CHANGELOG.md。 |
| v2.97.0 | 2026-07-17 | **9-issue drain：5 cluster PR（#259–#263）**。Added: **verify profiles**（[#258](https://github.com/PsychQuant/issue-driven-development/issues/258)）— `--profile code/prose/academic` + `--file`/`--dir` 輸入源 + file SHA-256 freshness gate（#228 非 git 等價物）、config `verify_profiles` 自訂；**`/idd-find`**（[#139](https://github.com/PsychQuant/issue-driven-development/issues/139)）— open+closed 全語料 relevance 查找，surfacing-only family 第 3 員；**dashboard comment 契約**（[#133](https://github.com/PsychQuant/issue-driven-development/issues/133)）+ **`idd-report --rollup`**（[#134](https://github.com/PsychQuant/issue-driven-development/issues/134)）— 人類敘事快照（只綁 phase 轉換，anti-#116）與四分組 attention 視圖（pull-only）；**`sdd_bias` 開關**（[#252](https://github.com/PsychQuant/issue-driven-development/issues/252)）— 硬閘出口 high 升 Spectra、default 逐 byte 不變；**Layer V deferred-record**（[#120](https://github.com/PsychQuant/issue-driven-development/issues/120)）；**surfacing-primitives family 文件（D12 軸）**（[#140](https://github.com/PsychQuant/issue-driven-development/issues/140)）。Changed: **model 世代同步**（[#251](https://github.com/PsychQuant/issue-driven-development/issues/251)）— codex-call default 升 `gpt-5.6-sol`（live probe 驗證）、全樹世代中立、idd-route candidate 更名 `codex-xhigh`；docs path catalog 補完（[#122](https://github.com/PsychQuant/issue-driven-development/issues/122)）。5 新 drift-guard suites，聚合器 36 suites 0 fail。詳見 CHANGELOG.md。 |
| v2.96.0 | 2026-07-13 | **gh-egress 硬化 cluster + idd-edit batch 語意**。**#227** exit-code 帶 ≥10（wrapper 絕不自行 exit <10 — `$?` 即可分流 gate-refusal vs gh-failure）；**#225** python3 統一 content-net 掃描（消滅 jq/no-jq 分歧；sensitive-key taxonomy；fail-closed 寬網）；**#226** Phase 2 rollout（6 skill 的 comment/edit egress 全接線 gh-egress — #117 mention net 機械生效於 comment channel）；**#158** idd-edit batch × R5 per-comment refuse + 繼續（outcome report + `✗ REFUSED` 顯著行，任一 refused → exit 4）。（[#227](https://github.com/PsychQuant/issue-driven-development/issues/227) [#225](https://github.com/PsychQuant/issue-driven-development/issues/225) [#226](https://github.com/PsychQuant/issue-driven-development/issues/226) PR #256；[#158](https://github.com/PsychQuant/issue-driven-development/issues/158) PR #257）詳見 CHANGELOG.md。 |
| v2.95.0 | 2026-07-11 | **Discussions intake bridge**（[#221](https://github.com/PsychQuant/issue-driven-development/issues/221)，PR #255，Spectra `discussions-intake-bridge`）— `idd-list --discussions`（opt-in surface：probe→Q&A/Ideas+未答 filter→dedup→actionable 區塊）+ `idd-issue --from-discussion`（Provenance verbatim blockquote + draft-and-confirm 回文，unattended 絕不 post）；**絕不 auto-file**。**idd-verify diff-freshness gate**（[#228](https://github.com/PsychQuant/issue-driven-development/issues/228)，PR #253）— FROZEN_SHA 比對，stale-snapshot aggregate 拒絕 + in-flight 不 commit 紀律。**IDD_CALLER registry**（[#161](https://github.com/PsychQuant/issue-driven-development/issues/161)，PR #254）— 動態樹掃描 drift-guard。詳見 CHANGELOG.md。 |
| v2.94.0 | 2026-07-10 | **Selective git auto-tag**（[#85](https://github.com/PsychQuant/issue-driven-development/issues/85)，PR #250）。Added: `idd-issue` 打 `idd-{N}-baseline`（main HEAD = rollback anchor），`idd-verify` 在 Aggregate PASS 打 `idd-{N}-verified`（review snapshot，cluster-aware）。只這兩個 milestone（無 diagnose/plan/implement tag），tag namespace 保持乾淨；phase-level tag 刻意留手動。新 config `auto_tag`（`enabled` **預設 ON**、opt-out via `enabled:false`、`baseline_format`/`verified_format`/`push_remote`），tag 含 `git push`（repo-wide side effect，已明文標註）；idempotent（tag 已存在→skip）+ graceful-skip on push fail（never aborts）。新增 `auto-tag` drift-guard（27 assertions）。詳見 CHANGELOG.md。 |
| v2.93.1 | 2026-07-09 | **Collaborator identity registry in idd-config**（[#86](https://github.com/PsychQuant/issue-driven-development/issues/86)，PR #247）。Added: optional `collaborators[]` config field — 把人的 alias / email / display-name → GitHub `@login` **不用猜**（`github_login` 必填；`email` 是 PII，只放 private/gitignored config）。`tagging-collaborators.md` Step 2.5 先查 registry（accelerator，非 authority — 命中仍經 `gh api users/<login>` 存在性驗證，未命中 fall through 到 API fuzzy-match）；`idd-config validate` 查 login charset + `aliases` 全域唯一 + PII 提醒。新增 `collaborators-schema` drift-guard（12 assertions）。**Also bundled: discussion-metadata helper**（[#141](https://github.com/PsychQuant/issue-driven-development/issues/141) + [#142](https://github.com/PsychQuant/issue-driven-development/issues/142)，PR #248）— idd-issue Step 3.5 surface 討論型 issue 的 assignee / label / milestone（`--discussion` flag + 預設 off 的 body heuristic；surface-only，unattended 絕不 auto-set；`blocked-by` deferred）；#142 label existence-check 防 blind add。`discussion-metadata` drift-guard（14 assertions）。詳見 CHANGELOG.md。 |
...
| `parallel-ai-agents`（pai） | **Canonical engine + 版本閘門 ≥ 2.19.0**（#207/#264，STABLE external-consumer contract 含 codexModel/codexEffort） | `idd-verify` 的 6-AI ensemble 引擎（4 lenses + Devil's Advocate + Codex 編排）+ `bin/codex-call` executable（#264 起 IDD 不再 vendor） | 兩層 degrade：canonical → manual fan-out（每層印 notice；vendored fork 已刪 #219/#264） |
...
| `codex-pro`（codex-pro marketplace） | **Hard dependency**（#264）— codex 治理 single source（`defaults.json` + profile.yaml 契約，MIN 0.7.0） | Verify 跨模型盲驗 lens 的 model/effort/max-time 治理（gpt-5.x，世代 pin 只在 codex-pro 一處） | 缺席 **fail-fast abort** + 一步安裝指令；絕不靜默 fallback 舊 model |
...
- ChatGPT Pro account (for the Codex cross-model lens, gpt-5.x — model governed by codex-pro's defaults.json)
...
### IDD ↔ superpowers stage mapping (#111)

IDD pipeline stage 對照 superpowers counterpart。**pre-implementation staging（brainstorm → written plan）委派 superpowers，IDD 不自建對等 skill**（依 deep-integration-over-hardcode：superpowers 既是 hard dependency，hand-off pointer 必然可解析，自建 = vendored fork）。verify ensemble 與 close audit trail 標為 **IDD 獨有 — 無 superpowers 對應**。

| IDD stage | superpowers counterpart | 說明 |
|-----------|-------------------------|------|
| `idd-issue`（問題框定）| （無 — issue framing 是 IDD 入口）| well-documented issue |
| `idd-diagnose`（bug RCA）| `superpowers:systematic-debugging` | RCA 執行框架（delegate，#209）|
| pre-implementation staging（brainstorm）| `superpowers:brainstorming` | design-heavy issue 的**非綁定 hand-off pointer**（idd-issue summary step + idd-diagnose 印出；使用者自選是否跟進，IDD 自身不 invoke）|
| written plan | `superpowers:writing-plans` | 同上 staging；IDD 的 `idd-plan` 是 approval-gate 版、不 delegate |
| `idd-implement`（TDD）| `superpowers:test-driven-development` | TDD 執行紀律（delegate，#209）|
| `idd-verify` | **IDD 獨有 — 無 superpowers 對應** | 6-AI ensemble cross-verify |
| `idd-close` | **IDD 獨有 — 無 superpowers 對應** | closing summary audit trail + two-tier gate |

> **hand-off pointer ≠ delegation（#111 契約）**：上表的 `superpowers:brainstorming` / `writing-plans` 是 **hand-off pointer**，不是 delegation。界線：
> - **delegation**（IDD flow **內部**呼叫 superpowers skill）：僅 `idd-implement` TDD 與 `idd-diagnose` RCA 的 process-discipline 走 delegation（#209）。planning discipline（`idd-plan` / Spectra，含對應的 `brainstorming` / `writing-plans`）**禁止** delegation。
> - **hand-off pointer**（idd-issue / idd-diagnose **印給使用者**的非綁定建議，**允許**）：IDD 自身**不** invoke 該 skill，使用者自選是否跟進。
>
> 機械檢查（drift-guard 硬鎖，見 `scripts/tests/superpowers-staging/`）：`grep -rn 'superpowers:' plugins/issue-driven-dev/skills/idd-plan/` **零命中** —— `idd-plan` 內不得出現任何 superpowers 呼叫；pointer 只活在 idd-issue / idd-diagnose 的 prose 建議裡。
