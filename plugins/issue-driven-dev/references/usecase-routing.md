...
| **27** | N 個 unrelated open issues 想一次解(無 emergent 關係) | **目前無內建 bulk-solve** — 手動 `idd-diagnose #N #M ...`(batch)→ per-issue `/idd-all #N` 或 `/idd-all-chain #N`;parked feature [#37](https://github.com/PsychQuant/issue-driven-development/issues/37) | (none — 用既有 atomic skills 組合)| #37 closing summary(parked,not superseded by [#44 chain-solve](https://github.com/PsychQuant/issue-driven-development/issues/44))|
...
> - **`/idd-all-chain` 不**滿足 #37 reopen criteria 的「一次解所有 open issue」— chain 不是 bulk,user 仍需手動挑 root(N=1 或 N>1)。#37 reopen 條件繼續成立(尚未 trigger)。
