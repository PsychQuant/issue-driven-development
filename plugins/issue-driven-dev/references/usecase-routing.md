...
| **27** | N 個 unrelated open issues 想一次解(無 emergent 關係) | **目前無內建 bulk-solve** — 手動 `idd-diagnose #N #M ...`(batch)→ per-issue `/idd-all #N` 或 `/idd-all-chain #N`;parked feature [#37](https://github.com/PsychQuant/issue-driven-development/issues/37) | (none — 用既有 atomic skills 組合)| #37 closing summary(parked,not superseded by #44 chain-solve)|

> **Chain-solve vs bulk-solve semantic 差異**(per #49 cross-link):
> - **`/idd-all-chain #N`**(v2.55.0+,row 25):**hot-chain from active work** — 1 root + auto-emergent spawn(sister bug / verify follow-up / mid-plan tangential / sister concern detected via spawn manifest)。Bounded by hard caps depth=2、max-issues=5。Reviewer mental model:「this thread is complete」。
...
> - **`/idd-all-chain` 不**滿足 #37 reopen criteria 的「一次解所有 open issue」— chain 不是 bulk,user 仍需手動挑 root。#37 reopen 條件繼續成立(尚未 trigger)。
> - 計畫中的 [#46 multi-root chain](https://github.com/PsychQuant/issue-driven-development/issues/46)(`/idd-all-chain #N #M #K`)會**部分 cover** #37 use case(若 user 願意手動 list 所有 roots),但仍非 zero-arg auto-sequence。
