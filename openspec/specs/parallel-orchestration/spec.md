...
Defines the **conflict-class discipline** for safely draining a backlog of independent issues — layered onto existing `idd-diagnose` + `idd-all` rather than a standalone orchestrator skill (per issue-driven-development#182's Non-Goal, confirmed when the 6-AI verify ensemble caught the standalone-skill design freezing a `SHALL` on a non-existent concurrency primitive). Governs: the **A–E conflict-class taxonomy** (`A_parallel_safe` / `B_resource_serialize` / `C_shared_module_coord` / `D_diagnose_first` / `E_verified_close`), classified by the physical resources an issue's implementation touches, with same-file issues grouped and `B`/`C` serialized per *named* resource; the `### Conflict Class` Diagnosis field contract that `idd-diagnose` emits and `idd-all` consumes (absent/unparseable defaults to `D_diagnose_first`, surfaced, never a parallel default; orthogonal to `### Complexity`); the opt-in parallel-diagnose fan-out (the one half genuinely concurrent today — N read-only investigators via the Workflow tool + a synthesis agent citing ≥2 legs); the `idd-all` multi-issue batch mode, a conflict-class-**ordered sequential** backlog drain; and the explicit **deferral of concurrent stateful execution** — no within-window concurrent-lane primitive exists, so the taxonomy is a forward-looking safety contract for manual or future parallelization, not a live auto-concurrency engine.
