# north-star-tracker Specification

## Purpose

Defines the **north-star tracker** convention for an *ordered, progressively-emerging* multi-stage roadmap — the cell of IDD's multi-issue-structure design space that neither `--bundle-mode` (ordered but upfront-all-children) nor milestone-first (`#83`, flat) covers. Governs: the `## Roadmap` checklist format on a `north-star`-labeled tracker issue, where an unfiled stage is a plain bullet and a filed stage is `- [x] Stage N: … → #M` (the `#M` reference is the emerged-vs-not signal); the file-on-start emergence rule (a stage becomes its own full-lifecycle issue only when work begins, never upfront); and the shared `idd-list` tracker-phase display rule that surfaces a tracker (`north-star`/`epic` label) as `tracking` with roadmap progress `<filed>/<total> stages` instead of the misleading `(no phase)` + `/idd-update` suggestion — a fix shared with `#83`'s milestone-tracked epics.
