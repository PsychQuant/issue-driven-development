# idd-issue-bundle Specification

## Purpose

Define bundle-creation flags for the `idd-issue` skill — `--parent`, `--blocked-by`, and `--bundle-mode` — that let a single invocation create a parent epic plus a chain (or set) of child issues with task-list registration and dependency annotation. The capability is orthogonal to the existing milestone, sister-sweep, and cross-repo `groups` mechanisms: bundle flags add hierarchy and ordering within a single target repo without disturbing those upstream/downstream behaviors. Sourced from change `add-bundle-flags-to-idd-issue`.
